...
for epoch in tqdm(range(epochs)):
    ...
    if (epoch + 1) % 20 == 0:
        torch.save(
            model.state_dict(),
            f"checheckpoint_path0_epoch_standard{epoch+1:02d}.pth"
        )
...
torch.save(
    model.state_dict(),
    f"checkpoint_path0_epoch_standard{epoch+1:02d}.pth"
)
# Save the losses
np.save("train_losses_path0_standard.npy", train_losses)
np.save("test_losses_path0_standard.npy", test_losses)
np.save("mse_list_path0_standard.npy", mse_list)
...
# 5) save to file
np.save("predictions_path0_standard.npy", all_preds)
...
ax_l1.grid()
fig_l1.savefig("l1_loss_path0_standard.png")
...
ax_mse.grid()
fig_mse.savefig("mse_path0_standard.png")
